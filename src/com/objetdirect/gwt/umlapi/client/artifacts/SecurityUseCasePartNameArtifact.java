...
import com.objetdirect.gwt.umlapi.client.editors.SecurityUseCasePartNameFieldEditor;
...
public class SecurityUseCasePartNameArtifact extends NodePartArtifact{

	private final UMLSecurityUseCase uMLsecurityusecase;
    ...
    public void buildGfxObject(){
    	...
    	GfxManager.getPlatform().addToVirtualGroup(this.gfxObject, this.nameEllip);
     	GfxManager.getPlatform().setFillColor(nameEllip, ThemeManager.getTheme().getSelectBoxForegroundColor());//getTheme()==NORMAL 薄い青
...
//    }
	...
	public void computeBounds() {
		...
		this.nameText = GfxManager.getPlatform().buildText(this.uMLsecurityusecase.getName(),
	...
	public void edit(final GfxObject editedGfxObject) {
		...
		final SecurityUseCasePartNameFieldEditor editor = new SecurityUseCasePartNameFieldEditor(this.canvas, this, isTheStereotype);
		String edited;
		if (isTheStereotype) {
			edited = this.stereotype.replaceAll("»", "").replaceAll("«", "");
		} else {
			edited = this.uMLsecurityusecase.getName();
		}
	...
	public String getSecurityUseCaseName() {
		return this.uMLsecurityusecase.getName();
	}
	...
	public void setSecurityUseCaseName(final String securityusecaseName) {
		this.uMLsecurityusecase.setName(securityusecaseName);
	}
	...
	@Override
	public String toURL(){
		return this.getSecurityUseCaseName() + "!" + this.getStereotype();
}
	...
	private Command deleteStereotype() {
		return new Command() {
			public void execute() {
				SecurityUseCasePartNameArtifact.this.stereotype = null;
				SecurityUseCasePartNameArtifact.this.nodeArtifact.rebuildGfxObject();
			}
		};
	}
	...
	private Command createStereotype() {
		return new Command() {
			public void execute() {
				SecurityUseCasePartNameArtifact.this.edit();
			}
		};
	}
	...
	private Command editCommand(final GfxObject gfxo) {
		return new Command() {
			public void execute() {
				SecurityUseCasePartNameArtifact.this.edit(gfxo);
			}
		};
	}
}