...
import com.objetdirect.gwt.umlapi.client.editors.AssetPartNameFieldEditor;
...
public class AssetPartNameArtifact extends NodePartArtifact{

	private final UMLAsset uMLasset;
    ...
    public void buildGfxObject(){
    	...
    	GfxManager.getPlatform().addToVirtualGroup(this.gfxObject, this.nameRect);
     	GfxManager.getPlatform().setFillColor(nameRect, ThemeManager.getTheme().getClassBackgroundColor());//getTheme()==NORMAL 白
//    	GfxManager.getPlatform().setFillColor(nameEllip, ThemeManager.getTheme().getClassForegroundColor());//getTheme()==NORMAL　黒　楕円の中身

    	GfxManager.getPlatform().setStroke(this.nameRect, ThemeManager.getTheme().getClassForegroundColor(), 1);//線の太さ
...
//    }
	...
	public void computeBounds() {
		...
		this.nameText = GfxManager.getPlatform().buildText(this.uMLasset.getName(),
	...
	public void edit(){
		if ((this.stereotype == null) || this.stereotype.equals("")) {
			this.stereotype = "«SecurityProperty»";
	...
	public void edit(final GfxObject editedGfxObject) {
		...
		final AssetPartNameFieldEditor editor = new AssetPartNameFieldEditor(this.canvas, this, isTheStereotype);
		String edited;
		if (isTheStereotype) {
			edited = this.stereotype.replaceAll("»", "").replaceAll("«", "");
		} else {
			edited = this.uMLasset.getName();
		}
	...
	public void setAssetName(final String assetName) {
		this.uMLasset.setName(assetName);
	}
	...
	public GfxObject getOutline() {
		...
		final GfxObject ellip = GfxManager.getPlatform().buildRect(this.nodeWidth, this.getHeight());
	...
	@Override
	public String toURL(){
		return this.getAssetName() + "!" + this.getStereotype();
}
	...
	private Command deleteStereotype() {
		return new Command() {
			public void execute() {
				AssetPartNameArtifact.this.stereotype = null;
				AssetPartNameArtifact.this.nodeArtifact.rebuildGfxObject();
			}
		};
	}
	...
	private Command createStereotype() {
		return new Command() {
			public void execute() {
				AssetPartNameArtifact.this.edit();
			}
		};
	}
	...
	private Command editCommand(final GfxObject gfxo) {
		return new Command() {
			public void execute() {
				AssetPartNameArtifact.this.edit(gfxo);
			}
		};
	}
