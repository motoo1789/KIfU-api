...
public class UseAndMisUseRelationLinkArtifact extends RelationLinkArtifact {
	 ...
	 */
	public UseAndMisUseRelationLinkArtifact(final UseCaseArtifact left, final MisUseCaseArtifact right, final LinkKind relationKind) {
		...
		switch (part) {
			case NAME:
				...
				if ((name == null) || name.equals("")) {
					defaultText = this.usecaseArtifact.getName() + "-" + this.misusecaseArtifact.getName();
				} else {
					defaultText = name;
				}
				break;
	...
	@Override
	public String toURL() {
		return "UseAndMisUseRelationLink$<" + this.usecaseArtifact.getId() + ">!<" + this.misusecaseArtifact.getId() + ">!" + this.relation.getLinkKind().getName()
	...
	protected void buildGfxObject() {
		...
		if (isComputationNeededOnLeft && isComputationNeededOnRight) {
			linePoints = GeometryManager.getPlatform().getLineBetween(this.usecaseArtifact, this.misusecaseArtifact);
