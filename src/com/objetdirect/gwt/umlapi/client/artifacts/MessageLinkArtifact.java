...
public class MessageLinkArtifact extends LinkArtifact {
	...
	public MenuBarAndTitle getRightMenu() {
		...
		rightMenu.setName(this.message.getLinkKind().getNameInMenu() + " " + this.leftLifeLineArtifact.getContent() + " "
		...
		rightMenu.addItem(EDIT.getMessage(), this.editCommand());
		rightMenu.addItem(REVERSE.getMessage(), this.reverseCommand(this.message));
		...
		for (final LinkKind messageKind : LinkKind.values()) {
			if (messageKind.isForDiagram(Type.SEQUENCE)) {
				linkSubMenu.addItem(messageKind.getNameInMenu(), this.changeToCommand(this.message, messageKind));
			}
		}
